 ...
 */
fun lengthInMeters(sagenes: Int, arshins: Int, vershoks: Int) = 4.445 * (vershoks + 48 * (arshins / 3.0 + sagenes)) / 100.0
 ...
 */
fun angleInRadian(grad: Int, min: Int, sec: Int) = toRadians(grad + (min + sec / 60.0) / 60.0)
...
fun accountInThreeYears(initial: Int, percent: Int): Double {
    val yearsCnt = 3
    var curRemainder: Double = initial.toDouble()
    for (i in 1..yearsCnt) curRemainder *= 1 + percent / 100.0
    return curRemainder
}
 ...
 */
fun numberRevert(number: Int) = number / 100 + number / 10 % 10 * 10 + number % 10 * 100
